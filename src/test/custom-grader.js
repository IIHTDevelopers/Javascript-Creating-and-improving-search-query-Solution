...
// Function to check if the search query is created properly
function checkSearchQueryCreation() {
    let result = 'Pass';
    let feedback = [];
    ...
    // Check if the URL with query strings is being constructed
    if (!studentCode.includes('q=') || !studentCode.includes('_page=')) {
        result = 'Fail';
        feedback.push("You must create a search query with query strings (_page, _limit, q).");
    }

    // Detailed logging of the check
    console.log(`\x1b[33mChecking search query creation\x1b[0m`);
...
// Function to check if the search results are filtered by the query
function checkSearchResultsFiltering() {
    let result = 'Pass';
    let feedback = [];
    ...
    // Check if the search results are being filtered by the query
    if (!studentCode.includes('.filter(post => post.title.toLowerCase().includes(query.toLowerCase()))')) {
        result = 'Fail';
        feedback.push("You must filter the search results based on the query.");
    }

    // Detailed logging of the check
    console.log(`\x1b[33mChecking search results filtering\x1b[0m`);
...
// Function to check if the search results are sorted correctly
function checkSearchResultsSorting() {
    let result = 'Pass';
    let feedback = [];
    ...
    if (!studentCode.includes('.sort((a, b) => a.title.localeCompare(b.title))')) {
        result = 'Fail';
        feedback.push("You must sort the search results by title.");
    }

    // Detailed logging of the check
    console.log(`\x1b[33mChecking search results sorting\x1b[0m`);
...
function checkErrorHandling() {
    ...
    if (!studentCode.includes('try {') || !studentCode.includes('catch (error)')) {
        result = 'Fail';
        feedback.push("You must use try/catch to handle errors during the fetch operation.");
    }
...
function gradeAssignment() {
    ...
    testResults.testCaseResults[GUID] = checkSearchQueryCreation();
    testResults.testCaseResults[GUID + '-results-filtering'] = checkSearchResultsFiltering();
    testResults.testCaseResults[GUID + '-results-sorting'] = checkSearchResultsSorting();
